...
#+ setup, include=FALSE, echo=FALSE, eval=T,echo = TRUE
library(rmarkdown)
library(knitr)
library(here)
...
# if(!isknitr) rmarkdown::render(knitr::spin(here('src/R/Modeling/run_degmodel_dropout.R'),knit=F),output_dir=here('Reports'),knit_root_dir=here())
...
#' Next, we'll load all the stan files we need
#+ loadstanfiles, echo = TRUE, include = T
#set up stan
...
ribo_sigma <- (readRDS)('data/ribo_sigma.rds')
...
stopifnot(all(uids4stan%in%rownames(ribo_sigma)))
...
get_dp_standata <- function(sel_uprotein_id,
  ...
  standata = list(
    ...
    voom_sigma=ribo_sigma[sel_uprotein_id,ribocols,drop=F],
    mybs = mybs,
    mydbs = mydbs
  )
  standata
...
satb2fits<-replicate(simplify=F,20,{
  ...
  optres<-safely(rstan::optimizing)(dp_model,data=get_dp_standata('ENSMUSP00000110057_4528'),algorithm='Newton',as_vector=FALSE,hessian=TRUE)
  Sys.time() - pre
  optres
})
