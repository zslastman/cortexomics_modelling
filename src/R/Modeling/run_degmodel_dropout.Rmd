...
## Setup
```{r}
knitr::opts_chunk$set(echo=T,cache=F,root.dir='/fast/groups/ag_ohler/work/dharnet_m/cortexomics_modeling/',dev='svg',message=F,warning=F)
```
...
base::source(here('src/R/Functions/rstan_functions.R'))
...
This works acceptably. So indeed, our model is doing what it's supposed to, and recovering paramters from simulated data at least in the case of increasing protein, where it aught to.
...
satb2fits<-replicate(simplify=F,5,{
	pre = Sys.time()
	optres<-safely(rstan::optimizing)(dp_model,data=get_dp_standata_withpriors('ENSMUSP00000110057_4528',ribomatrscl=rnamatrscl,rna_sigma),algorithm='Newton',as_vector=FALSE,hessian=TRUE)
	Sys.time() - pre
	optres
})
...
```{r, singlegenesampling,results='hide'}
...
TODO: Get a trace plot of each parameter.
...
```{r, singlegeneposteriorplot,include=T,eval=T,fig.height=12,results='asis',dev='png'}

# mcmcpairplot <- mcmc_pairs(sampling,pars=vars(matches('lKs|sigma|st|pi|(prot\\[1,2\\])|(prot\\[1,3\\])|lp__')),save_gg_objects=TRUE,off_diag_args=list(size=0.1))
mcmcpairplot
# plot(1)

# allpars <- sampling%>%as.data.frame%>%colnames
# plot(sampling,vars=allpars%>%str_subset('lKs|sigma|st|pi|(prot\\[1,2\\])|(prot\\[1,3\\])|lp__'))
# rstan::plot(sampling,vars=allpars%>%str_subset('pi'))

mcmc_trace(sampling,pars=vars(matches('lKs|sigma|st|pi|(prot\\[1,2\\])|(prot\\[1,3\\])|lp__')))
...
```{r approxvis,message=F,warning=F,results='hide'}
...
optres$par[colnames(optres$hessian)] + 1.96*sqrt(diag( (-optres$hessian)))
...
################################################################################
selgene = satb2uid
geneoptsfile <- here('data/geneoptfits.rds')
file.remove(geneoptsfile)
if(!file.exists(geneoptsfile)){
  #
  require('R.utils')
  genefits <- list()
  #
  genefits <- mclapply(mc.cores=20,uids4stan,(function(selgene){
    ...
    selgenefits<-lapply(rep(selgene,5),safely(function(selgene){
      withTimeout(timeout=15,{
        # message(selgene)
        optdat <- get_dp_standata_withpriors(selgene)
        optdat$lMS
  ...
  saveRDS(genefits,file=geneoptsfile)
}else{
  genefits<- readRDS((file=geneoptsfile))
}
