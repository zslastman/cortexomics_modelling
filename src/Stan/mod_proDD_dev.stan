...
parameters {
  ...
  matrix<lower=-20,upper=20>[G,T] p_dev;  //log protein deviance from the trajectory
...
model {
  ...
  ribnorm ~ normal(0,3);
  protnorm ~ normal(0,3);
  p_dev ~ student_t(l_ribo_priornu,0,l_ribo_priorsd);
  
  {
    ...
    for(i in 1:G){
      ...
      for(j in 1:nsamples){
        if(is_inf(lMS[i, j])){
          target += normal_lccdf(prot[i, experimental_design[j]] + p_dev[i,experimental_design[j] ] | rho[j], fabs(zetastar[counter]));
          counter += 1;
        }else{
